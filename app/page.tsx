  ...
  const handleCommander = async () => {
    ...
    const { error } = await supabase
     ...
     .insert({
        ...
        product_name: produitActuel.nom,
        price_text: produitActuel.prixTexte,
        status: 'active',
    ...
    let msg = `Salut Apay Digital CG! Je viens de commander : ${produitActuel.nom} - ${produitActuel.prixTexte}.\nNom: ${prenom} ${nom}`
    if (user.email) msg += `\nEmail: ${user.email}`
        ...
        <header className="sticky top-0 z-50 bg-gray-950/80 backdrop-blur border-b border-gray-800">
          <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              {user? (
                <>
                  <a href="/compte" className="text-gray-300 hover:text-white font-semibold px-4 py-2 rounded-lg text-sm">
                    Mon compte
                  </a>
                  <button
            ...
            <div className="bg-gray-900 p-6 rounded-lg w-full max-w-md border border-gray-700">
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  ...
                  className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-700 py-3 rounded font-semibold"
                >
                  {commandeLoading? 'Création...' : 'Envoyer sur WhatsApp'}
                </button>
              </div>
            </div>
          </div>
